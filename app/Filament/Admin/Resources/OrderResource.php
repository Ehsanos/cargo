...
//                Tables\Columns\TextColumn::make('status')->label('حالة الطلب')
                ...
                Tables\Columns\TextColumn::make('receive_address')->formatStateUsing(fn($record,$state)=>trim($record->receive_address)??$record->receive_phone)->label('هاتف المستلم ')->description(fn($record) =>  ltrim($record?->receive_phone, '+'))
                    ->url(function($record) {
